...
echo "=================================================="
echo " FigureHue systemd 托管安装 (port $PORT)"
echo "=================================================="
...
chmod +x "$WORKDIR/figurehue_healthcheck.sh" "$WORKDIR/scripts/run_server.sh"
...
systemctl --user enable figurehue.service figurehue-healthcheck.timer
systemctl --user restart figurehue.service
systemctl --user start figurehue-healthcheck.timer

echo ""
echo "状态:"
systemctl --user status figurehue.service figurehue-healthcheck.timer --no-pager || true
echo ""
echo "下次探活时间:"
systemctl --user list-timers figurehue-healthcheck.timer --no-pager || true
echo "=================================================="
echo " 完成。本地访问: http://127.0.0.1:${PORT}"
echo " 日志: tail -f $WORKDIR/web_log.txt"
echo " 手动重启: bash $WORKDIR/restart.sh"
echo " 手动探活: $WORKDIR/figurehue_healthcheck.sh"
echo "=================================================="
